let MainBoard = function(){
    ...
    /**
     * generateMapObstacles place Obstacles on the map.
     ...
     */
    this.generateMapObstacles = function(){
    ...
    this.clearMovementCheck = function(){
        this.removeClassName("movementPossible");
        this.removeClassName("movementImpossible");
    };
    /**
     * showMovementPossible add movementPossible to each possible cells where the player can move.
     */
    this.showMovementPossible = function(player){
        ...
        for(let positionLoop = 0; positionLoop < positions.length; positionLoop ++){
            this.getGridElement(positions[positionLoop].line, positions[positionLoop].column).classList.add("movementPossible");
        }
    };
    /**
    * showMovementImpossible add movementImpossible to each impossible cells where the player cant move.
    */
    this.showMovementImpossible = function(player){
        let positions = this.getImpossibleNewPositions(player);
        for(let positionLoop = 0; positionLoop < positions.length; positionLoop ++){
            this.getGridElement(positions[positionLoop].line, positions[positionLoop].column).classList.add("movementImpossible");
        }
    };
    ...
    this.addPlayerElement = function(line,column){
        ...
        if(this.boardObjects.playerOne === null){
            player.element.className ="player playerOne";
